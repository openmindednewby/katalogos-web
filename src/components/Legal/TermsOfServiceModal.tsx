 ...
 *
 * Displays placeholder legal text for the terms of service in a full-screen modal.
...
export const TermsOfServiceModal = ({ visible, onClose }: TermsOfServiceModalProps): React.ReactElement => {
  ...
  return (
    ...
    >
      <View style={[styles.container, { backgroundColor: colors.background }]} testID={TestIds.TERMS_OF_SERVICE_SCREEN}>
        <ScrollView
          accessibilityViewIsModal
          aria-label={FM('legal.termsOfService.title')}
          contentContainerStyle={styles.scrollContent}
          role="dialog"
        >
          <View style={styles.headerRow}>
            <Text style={[styles.title, { color: colors.text }]}>
              {FM('legal.termsOfService.title')}
            </Text>
            <TouchableOpacity
              ...
              testID={TestIds.TERMS_OF_SERVICE_CLOSE}
              onPress={onClose}
            >
          ...
          <Text style={[styles.lastUpdated, { color: colors.textSecondary }]}>
            {FM('legal.termsOfService.lastUpdated', PLACEHOLDER_DATE)}
          </Text>

          {SECTIONS.map((key) => (
            <LegalSection
              key={key}
              body={FM(`legal.termsOfService.${key}.body`)}
              title={FM(`legal.termsOfService.${key}.title`)}
            />
          ))}
        </ScrollView>
      </View>
