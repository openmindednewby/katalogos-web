/**
 * Tests for useStartExperiment hook.
...
const TEST_EXPERIMENT_ID = 'exp-start-1';
...
describe('useStartExperiment', () => {
  ...
  it('calls customInstance with POST and correct URL', async () => {
    mockCustomInstance.mockResolvedValue({ id: TEST_EXPERIMENT_ID, status: 'Running' });
    const onSuccess = jest.fn();

    const { result } = renderHook(
      () => useStartExperiment({ onSuccess }),
    ...
    expect(mockCustomInstance).toHaveBeenCalledWith(
      expect.objectContaining({
        url: `/api/v1/experiments/${TEST_EXPERIMENT_ID}/start`,
        method: 'POST',
      }),
    );
